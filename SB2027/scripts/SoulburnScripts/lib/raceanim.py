...
def speed_profile(spine, closed: bool = True, *,
                  lat_g: float | None = None,
                  v_min: float | None = None,
                  v_max: float | None = None,
                  brake_g: float | None = None,
                  accel_g: float | None = None,
                  surface: str = "dry") -> list[float]:
    """
    ...
    All physics constants default to the module-level values (modern high-downforce
    car on a dry surface) but can be overridden per call. ``surface`` is a
    convenience shortcut: ``"wet"`` multiplies the effective ``lat_g`` and
    ``brake_g`` by 0.75, which is a conservative approximation of grip loss on a
    wet but not standing-water surface. Explicit keyword arguments take precedence
    over the surface multiplier.
    """
    _lat_g  = LAT_G  if lat_g  is None else lat_g
    _v_min  = V_MIN  if v_min  is None else v_min
    _v_max  = V_MAX  if v_max  is None else v_max
    _brake  = BRAKE_G if brake_g is None else brake_g
    _accel  = ACCEL_G if accel_g is None else accel_g

    if surface == "wet":
        if lat_g   is None: _lat_g  = _lat_g  * 0.75
        if brake_g is None: _brake  = _brake  * 0.75
    elif surface != "dry":
        raise ValueError(f"surface must be 'dry' or 'wet'; got {surface!r}")

    radii = curvature_radius(spine, closed)
    v = [max(_v_min, min(_v_max, math.sqrt(_lat_g * G * r))) for r in radii]
    ...
    for _ in range(2):
        for i in range(n - 1, -1, -1):          # backward: braking
            j = (i + 1) % n
            limit = math.sqrt(v[j] ** 2 + 2.0 * _brake * G * seg[i])
            v[i] = min(v[i], limit)
        for i in range(n):                      # forward: traction
            j = (i + 1) % n
            limit = math.sqrt(v[i] ** 2 + 2.0 * _accel * G * seg[i])
            v[j] = min(v[j], limit)
    return v


def lap_time(spine, closed: bool = True, **profile_kwargs) -> float:
    """
    Seconds to complete a lap at the profiled speed.

    ``**profile_kwargs`` are forwarded to :func:`speed_profile`, so
    ``lap_time(spine, surface="wet")`` returns the wet-surface lap time.
    """
    n = len(spine)
    speeds = speed_profile(spine, closed, **profile_kwargs)
    total = 0.0
    for i in range(n):
        j = (i + 1) % n
...
def multilap_positions(spine, seconds: float, count: int = 20, *,
                       num_laps: int = 1,
                       gap_s: float = FIELD_GAP_S,
                       stagger_m: float = 3.0,
                       closed: bool = True,
                       surface: str = "dry") -> list:
    """
    Field positions for a multi-lap race at a given elapsed time.

    Extends :func:`field_at_time` to cover races longer than one lap. The
    leader's position is the distance travelled modulo the lap length, so the
    circuit repeats correctly. ``num_laps`` is the total race length in laps;
    cars that have finished their allocation of laps are omitted from the result.

    The elapsed time is measured from the race start (lights out), not from the
    start of any individual lap. Lap boundaries are not special: a car that
    crosses the start/finish line mid-frame does not teleport or pause.

    Returns ``(x, y, heading, roll, pitch, lap_number)`` per car, leader first.
    ``lap_number`` is 1-based (lap 1 = first lap) and is useful for a broadcast
    overlay or for triggering a lap-specific camera cut.

    Wet-surface grip is forwarded to :func:`speed_profile` so multi-lap lap
    times are consistent with single-lap calls using the same surface condition.
    """
    lap_s = max(lap_time(spine, closed, surface=surface), 1e-3)
    lap_len = lap_length(spine, closed)
    speeds = speed_profile(spine, closed, surface=surface)

    def _distance_at_t(t: float) -> float:
        """Distance into the *current* lap for a car at time ``t``."""
        lap_t = t % lap_s
        # Walk the speed profile for this lap's elapsed seconds.
        n = len(spine)
        seg = [math.dist(spine[i], spine[(i + 1) % n]) for i in range(n)]
        remaining = lap_t
        travelled = 0.0
        for i in range(n):
            ...
            dt = seg[i] / mean_v
            if dt >= remaining:
                return travelled + seg[i] * (remaining / max(dt, 1e-9))
            remaining -= dt
            travelled += seg[i]
        return lap_len   # at finish line

    def _placed(car_index: int, at_seconds: float):
        """World position for one car at one time."""
        d = _distance_at_t(at_seconds)
        x, y, heading = point_at(spine, d, closed=closed)
        side = 1.0 if car_index % 2 == 0 else -1.0
        a = math.radians(heading)
        nx, ny = math.cos(a), -math.sin(a)
        return (x + nx * side * stagger_m,
                y + ny * side * stagger_m,
                d,
                int(at_seconds / lap_s) + 1)   # lap number 1-based

    out = []
    for i in range(count):
        t = seconds - i * gap_s
        if t < 0.0:
            continue   # car has not started yet
        laps_done = int(t / lap_s)
        if laps_done >= num_laps:
            continue   # car has finished the race
        x, y, distance, lap_num = _placed(i, t)

        # Heading from car's own centred-difference path.
        back_x, back_y, _, _ = _placed(i, t - 0.06)
        fwd_x,  fwd_y,  _, _ = _placed(i, t + 0.06)
        dx, dy = fwd_x - back_x, fwd_y - back_y
        heading = (math.degrees(math.atan2(dx, dy)) % 360.0
                   if math.hypot(dx, dy) > 1e-6
                   else point_at(spine, distance, closed=closed)[2])
        ...
        roll, pitch = attitude_at(spine, distance, closed)
        out.append((x, y, heading, roll, pitch, lap_num))
    return out
